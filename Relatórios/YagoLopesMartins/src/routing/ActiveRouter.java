...
public abstract class ActiveRouter extends MessageRouter {
	...
	class Historico{
		...
		public Historico(double tempo,	int msg_id,	int msg_src,	int node_seen){
			this.tempo = tempo;
			this.msg_id = msg_id;
			this.msg_src = msg_src;
			this.node_seen = node_seen;
		}
	}
	class Reputacoes{
		int id_no;
		int reputacao;
		public Reputacoes(int id_no, int reputacao){
			this.id_no = id_no;
	...
	public void init(DTNHost host, List<MessageListener> mListeners) {
		...
		*/
		List<Reputacoes> reputacoes = new ArrayList<>();
		reputacoes.add(new Reputacoes(1, 2));

		List<Historico> historicos = new ArrayList<>();
			historico.add(new Historico(0.6, 2, 1, 3));
			historico.add(new Historico(0.6, 2, 1, 3));
			historico.add(new Historico(0.6, 2, 1, 3));

		for(Reputacoes rep : reputacoes){
			 this.reputacoes.add(50.0);
		}
	}
	/**
	...
	public void changedConnection(Connection con) {
		if (this.energy != null && con.isUp() && !con.isInitiator(getHost())) {
			...
			// se conectou com novo usuario
			if (con.isUp()){
				//funcao encontra no(B)
				DTNHost vizinho = con.getOtherNode(this.getHost);
				DTNHost eu = this.getHost();

				trocaHistorico(this.historico, vizinho.historico);

				/*
					List<Tuple<double tempo, Tuples de tuples> historicoVixinho  = vixinho.getHistorico();
	...
	// troca historico entre A e B
	public void trocaHistorico(){

		for(Historico his : historicos){
			if (historico.tempo >  last_contact(vizinho)){
					if(historico.msg_id == my_id){
							if(last_contact(historico.node_seen) > last_contact(vizinho)){
								if(historico.node_seen != historico.msg_id && DTNHost == me){
									reputacao -= x;
								}
							}
					}
	...
	public int receiveMessage(Message m, DTNHost from) {
		...
		// atualização de reputacoes
		if(from.getAddress() != m.getFrom()){
			reputacoes [from.getAddress] += x;
		}
	...
	public void update() {		
        ...
        System.out.println("----------------- TESTING IRONMAN -----------------");
